...
> **🎉 改名公告**: 项目原名 `whisper-input`,从 v1.0.0 起改名为 `daobidao`(叨逼叨)。
> 老命令 `pip install whisper-input` 会自动转发到新包,但**新功能更新都发布到
> `daobidao`**,推荐改用 `uv tool install daobidao`。详见
> [docs/29-改名为daobidao/SUMMARY.md](docs/29-改名为daobidao/SUMMARY.md)。
...
脚本会交互式选择语言，然后自动装好 `uv` / Python 3.12 / 系统依赖 / `daobidao` 本身，跑 `daobidao --init`（预下载约 990 MB 的 Qwen3-ASR 0.6B ONNX 模型，macOS 下同时安装 `~/Applications/Daobidao.app`），最后询问是否立即启动。重复执行安全，已装好的步骤会自动跳过，`uv tool install --upgrade` 会把 `daobidao` 升级到最新。
...
> **注意**：首次运行（或执行 `daobidao --init`）时，工具会在 `~/Applications/Daobidao.app` 安装一个极简 `.app` bundle。macOS 系统权限对话框和系统设置里显示的都是 "Daobidao"，直接给这个条目授权即可。完整卸载请先运行 `daobidao --uninstall`，再执行 `uv tool uninstall daobidao`。
...
首次运行 `daobidao` 会通过 `modelscope.snapshot_download` 自动从 ModelScope 国内 CDN 拉取 Qwen3-ASR 0.6B ONNX 模型（~990 MB），缓存到 `~/.cache/modelscope/hub/`。一次成功后永久离线。后续可在应用设置页切换到 1.7B 版本(会额外下载 ~2.4 GB)。
...
```bash
# 指定快捷键
daobidao -k KEY_FN          # macOS: Fn/Globe 键
daobidao -k KEY_RIGHTALT    # Linux: 右 Alt 键
...
整个项目采用 src layout,所有 Python 代码在 `src/daobidao/` 下,是一个
可 `uv sync` 编辑式安装的真 package。入口点是 console script
`daobidao`(等价于 `uv run python -m daobidao`)。

```
按住快捷键 → HotkeyListener (daobidao.backends) → AudioRecorder (sounddevice)
松开快捷键 → stt.Qwen3ASRSTT (onnxruntime) → InputMethod → 文本输入到焦点窗口
```

平台后端（`daobidao.backends`）运行时按 `sys.platform` 自动选择：
- **Linux**: evdev 读键盘事件 + xclip/xdotool 剪贴板粘贴
- **macOS**: pynput 全局键盘监听 + pbcopy/pbpaste + Cmd+V 粘贴

STT 推理层（`daobidao.stt.qwen3`）：
