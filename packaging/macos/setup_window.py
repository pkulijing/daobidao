...
三阶段：
  ...
  Stage B — whisper_input.stt.downloader 从 ModelScope 下载 SenseVoice ONNX
            （~231MB,5 个文件,已缓存自然秒过）
  Stage C — 起 python -m whisper_input,读日志直到 "[sensevoice] 模型加载完成"
            后退出窗口
...
class SetupWindow:
    ...
    def _stage_b_run(self) -> bool:
        ...
        # 自己的进程里(bundled python-build-standalone)运行,不需要起 user venv
        # 子进程 —— 下载、SHA256 校验、manifest 落盘全程 stdlib。
        # 本地已命中就直接返回,零联网。
        # 把 src/ 加到 sys.path 让 `import whisper_input` 能找到包(bundled
        # python 本身并没有 install whisper_input,只有 user venv 里才有)。
