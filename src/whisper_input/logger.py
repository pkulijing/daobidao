...
日志目录按平台分：
- macOS: ~/Library/Logs/Whisper Input/   (Apple 推荐;Console.app 会扫)
- Linux: $XDG_STATE_HOME/whisper-input/  (兜底 ~/.local/state/whisper-input/)
- Dev  : {repo_root}/logs/                (通过 .git + pyproject.toml 探测)

目录里有两个文件:
- whisper-input.log          app 的结构化日志(logfmt),RotatingFileHandler,
                             1 MB × 3 份
- whisper-input-launchd.log  macOS 专属,由 launchd 在 plist 里 StandardErrorPath
...
APP_LOG_FILENAME = "whisper-input.log"
LAUNCHD_LOG_FILENAME = "whisper-input-launchd.log"
...
def _dev_log_dir() -> Path | None:
    ...
    行为和 config 路径解析保持一致。
    """
    from whisper_input.config_manager import _find_project_root
...
def get_log_dir() -> Path:
    ...
    if IS_MACOS:
        return Path(
            os.path.expanduser("~/Library/Logs/Whisper Input")
        )
