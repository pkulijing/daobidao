...
**Future work / backlog** is tracked in **GitHub Issues** ([open issues](https://github.com/pkulijing/daobidao/issues)). Each issue carries three labels: `type:*` / `area:*` / `priority:*` (P0 必须做、P1 重大新功能、P2 一般小功能小修复). There is **no local index file** — the issue is the single source of truth, and the at-a-glance view is the [open-items saved query](https://github.com/pkulijing/daobidao/issues?q=is%3Aissue+is%3Aopen+label%3Apriority%3AP0%2Cpriority%3AP1%2Cpriority%3AP2) (`/triage` ranks them by priority × scope when picking the next round). Items we **deliberately decided not to do** are archived as closed issues labelled [`wontfix`](https://github.com/pkulijing/daobidao/issues?q=is%3Aissue+is%3Aclosed+label%3Awontfix) so a future reader hitting them in an old SUMMARY doesn't mistake them for oversights. New ideas during a round go straight to `gh issue create` (templates in `.github/ISSUE_TEMPLATE/`); per-round `SUMMARY.md` "后续 TODO" sections should reference an issue # rather than re-describing the idea inline.
...
uv run daobidao --verbose           # full structured logs on stderr, level DEBUG unless config.yaml sets log_level explicitly
uv run daobidao --quiet             # no terminal output at all
...
- **`backends/hotkey_linux.py`** — evdev keyboard monitoring with 300ms combo-key detection. `select()` is only used as an interruptible sleep: every wake drains **all** devices, because some USB HID keyboards' fds are never reported readable by `select()` (issue #18). Dead fds are pruned instead of killing the listener thread.
...
- **ModelScope distribution via `zengshuishui/Qwen3-ASR-onnx`**: community-maintained ONNX export of the official Qwen3-ASR weights. Single repo hosts both variants side-by-side under `model_0.6B/` and `model_1.7B/`; `allow_patterns` avoids pulling the wrong one. `snapshot_download` lands files somewhere under `~/.cache/modelscope/` — **never hardcode or recompute that path**. Since modelscope 1.40 the call delegates to `modelscope_hub` and the layout is chosen at runtime by its `find_reusable_legacy_repo_dir()` across four historical layouts (`models/<owner>--<name>/snapshots/<rev>/`, `hub/models/<owner>/<name>/`, …). The only trustworthy path is the one modelscope returns: `Qwen3ASRSTT.cache_root` after a successful `load()`, or the return value of `snapshot_download` itself (round 37 — `DownloadManager.set_cache_root`). The base `modelscope` pip package is only ~36 MB with minimal transitive deps — torch / transformers / scipy are hidden behind extras like `[framework]`, which we don't install.
- **Offline startup: ask modelscope whether the cache is enough before touching the network**: `Qwen3ASRSTT.load()` first calls `snapshot_download(..., local_files_only=True)` — a zero-network cache probe that returns **modelscope's own** cache root, so we still never compute the layout ourselves. That local-only branch only checks "directory non-empty" (`modelscope_hub`), so the probe double-checks `_required_cache_files(variant)` (the 3 ONNX + the three tokenizer files `Qwen3Tokenizer` actually opens) before trusting it; any miss falls through to the previous download path unchanged. This is what lets a warm-cache machine start with no network at all (a stale/broken GNOME proxy used to make startup hang or die). The probe triggers a benign "confirm the cached file is for revision" WARNING, logged by `modelscope` (<1.40: its own stderr handler with `propagate=False`, so it bypasses our console channel and even `--quiet`) or `modelscope_hub.download` (>=1.40). It is dropped by a message filter on both loggers for the probe's duration — a filter, not `setLevel`, because modelscope's `get_logger()` resets the level to INFO on every call. Consequence of the probe: once a variant is fully cached, `load()` never contacts ModelScope again, so upstream re-exports are **not** picked up automatically (see "Upgrading the Qwen3-ASR model").
...
- **Parsing machine output happens under a locked locale**: `recorder.py` shells out to `pactl` with `env={**os.environ, "LC_ALL": "C", "LANGUAGE": ""}`. `pactl` translates its field names (`Name:` → `名称：`, `Ports:` → `端口：`), so on a Chinese desktop the un-locked parser matched nothing and reported "no microphone" with a mic plugged in (round 37). Note `env=` replaces the environment wholesale — copy `os.environ` or pactl loses `XDG_RUNTIME_DIR` and can't reach the audio server. Same rule applies to any future shell-out we parse.
- **Console channel, not "all logs" or "no logs"**: `configure_logging(console=True)` (the default) attaches a plain-text stderr handler that passes **every WARNING+** plus an INFO allowlist of once-per-startup milestones (`_CONSOLE_INFO_EVENTS` in `logger.py`). `--verbose` swaps it for the full `ConsoleRenderer`; `--quiet` drops it. Round 34 had silenced the terminal completely, which made a 7-minute first-run model download look like a hang; round 37 restored feedback without bringing back INFO spam.
...
Model files for the 0.6B variant (~990 MB: 3 ONNX + tokenizer dir) are downloaded on first `Qwen3ASRSTT.load()` via `modelscope.snapshot_download` and land under `~/.cache/modelscope/` (layout picked by modelscope at runtime; read `cache_root` rather than assuming one). Switching to 1.7B downloads the second bundle (~2.4 GB) alongside. After one successful download the app is fully offline. Cache is managed by the `modelscope` library itself (content addressing + validity metadata).

## Upgrading the Qwen3-ASR model

Existing installs will **not** pick up a new upstream export on their own: `load()` returns early from the zero-network cache probe whenever all `_required_cache_files(variant)` exist, and only calls the online `snapshot_download` on a cache miss. The local-only branch ignores `revision=` (that is exactly what the benign WARNING says), so pinning a new revision alone does **not** force a refresh — either change the probe's acceptance criteria in code, or have users delete the cached bundle under the path shown by `Qwen3ASRSTT.cache_root`.

When the upstream ModelScope repo pushes a new ONNX export:

1. Test manually in a dev venv whether the new revision still works (on a cache miss, snapshot_download defaults to the repo's default branch — usually `master` — so a fresh download picks up the latest)
